...
import { getEffectiveAddressImm, getEffectiveAddressRegister, getRegisterNumber, isEffectiveAddress, isIType, isJType, isLabel, isMnemonic, isRegister, isRType, isValidRegister, isValue, isXBit, } from "./utils"
import { AssemblerError, AssemblerErrorList, ErrorType } from "./errors";


function getOperands(instruction: string): string[] {
    // split the string on a comma or space
    return instruction.split(/,|\s/).slice(1).filter((operand) => operand !== '');
}


export function encodeInstruction(instruction: string, labels: Map<string, number>, pc: number): InstructionR | InstructionI | InstructionJ {
    const mnemonic = instruction.split(" ")[0];
    const operands = getOperands(instruction);
    ...
    const expectedOperandCount = INSTRUCTION_SET[instructionOpcode].operands.filter((operand) => operand.type !== OperandType.UNUSED).length;
    ...
    let index = 0;
    INSTRUCTION_SET[instructionOpcode].operands.forEach((operand) => {
...
export function assemble(program: string): { memory: Memory, labels: Map<string, number> } | never {
    ...
    programLines.forEach((instruction) => {
        ...
        try {
            encodedInstruction = encodeInstruction(instruction, labels, pc);
    ...
    encodedInstructions.push(encodeInstruction("HALT", labels, pc) as InstructionJ);
